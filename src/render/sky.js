 ...
 *
 * The horizon stop is a uniform rather than a constant because it tracks the
 * fog, and the fog tracks the biome under the camera: the sky over the forest
 * is a cool grey-green and the sky over the desert is hot sand, and the change
 * between them is most of what makes the journey feel like one.
 *
 ...
 * exactly to the scene fog so the far hills dissolve instead of ending at a
 * visible line.
 *
...
export function createSky() {
  const material = new ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        // A tall haze band. A low sun puts a lot of dust in the air, and that
        // is what lets distant hills dissolve into the fog instead of
        // stacking up as silhouettes.
        vec3 lower = mix(horizonColor, middleColor, smoothstep(0.0, 0.22, h));
  ...
  return {
    dome,
    /** Keep the horizon stop equal to the scene fog, or the far hills end at a
     *  visible line where the dome meets them. */
    setHaze(colour) {
      material.uniforms.horizonColor.value.set(colour);
    },
  };
}
    ...
    new ShaderMaterial({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          // A low, heavy deck: the fade starts close to the horizon so cloud
          // hangs over the hills rather than sitting in a clear ring above
          // them.
